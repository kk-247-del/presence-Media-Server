...
// sessionId → { a: WebSocket, b: WebSocket, obstructionActive: bool, timer: null }
...
wss.on("connection", (ws, req) => {
  ws.isAlive = true;
  // LAW: Use the sub-protocol as the Session ID for Railway compatibility
  ws.sessionId = req.headers['sec-websocket-protocol']?.toUpperCase() || "LOBBY";

  log(`✨ Peer joined Window: ${ws.sessionId}`);

  let s = sessions.get(ws.sessionId);
  if (!s) {
    s = { a: ws, b: null, obstructionActive: false, timer: null };
    sessions.get(ws.sessionId) || sessions.set(ws.sessionId, s);
  } else if (!s.b) {
    s.b = ws;
